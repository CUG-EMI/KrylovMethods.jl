...
function qmr{T1,T2}(A::SparseMatrixCSC{T1,Int},b::Array{T2,1}; kwargs...) 
	Ax = zeros(promote_type(T1,T2),size(A,2))                  # pre-allocate
	return qmr(x -> A_mul_B!(1.0,A,x,0.0,Ax),b;kwargs...) # multiply with transpose of A for efficiency
end

qmr(A,b; kwargs...) =  qmr(x -> A*x,b; kwargs...)
...
function qmr(A::Function, b::Vector; tol::Real=1e-6, maxIter::Int=100, M=x->copy(x),
             x0::Vector=[],out::Int=0,storeInterm::Bool=false, tolRho::Real=1e-40)
